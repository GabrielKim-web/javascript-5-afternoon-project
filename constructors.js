...
// Write a constructor function called User. This function should take in 4 parameters called name, age, email, and this.savedPosts in that order. Name and email will be strings, age will be a number and this.savedPosts will be an array of objects. These objects will each have 3 properties: id (a number), title (a string), and rating (a number between 1 and 5). These objects are the posts that the user will have saved to their account.

// Once the User constructor function is created, write a prototype method for the User function. Name this method addSavedPost. It should take in three parameters: id (a number), title (a string) and rating (a number). Use these parameters to create a new object and add it to the this.savedPosts array. Make sure to name the properties the same as described previously (id, title, rating).
...
// Write a prototype method for the User constructor function named removeSavedPost that will take in one number parameter representing the post id. Use this id to find and remove the matching object in the this.savedPosts array.

// Code here
User.prototype.removeSavedPost = function(num) {
  const deleteThis = this.savedPosts.findIndex(element => {
    element.id == num
  })
  this.savedPosts.splice(deleteThis, 1);
}

////////// PROBLEM 7 //////////

// You will continue to use the constructor function you created in problem 5.
// Write a prototype method for the User constructor function named changePostRating that will take in two number parameters. The first will be an id (a number) and the second will be the new rating (a number). Use the id to find the matching object in the this.savedPosts array. Once you find the matching object, update it's rating score with the new rating parameter.
